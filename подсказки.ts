...
65 middleware param
routes.param("id" ,(req,res,next(),val) => {}) - мы при промежуточном программном обеспечении с param мы получаем четвертый параметр value
так вот это служит только для специальных параметров которые вы укажите в нашем случае id
66 ищем статически файлы с помощью express
app.use(express.static())
067 Environment Variables
NODE_ENV = development node dist/express.js - показывает в какой среде разработке мы работаем 
